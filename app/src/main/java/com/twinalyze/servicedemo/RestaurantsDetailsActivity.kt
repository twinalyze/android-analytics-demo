...
//        }
       ...
       /* // make sure it’s clickable
        ...
        // get prefs
        val prefs = getSharedPreferences("my_prefs", MODE_PRIVATE)   // Fragment: requireContext().getSharedPreferences(...)

        cartCount = prefs.getInt("cart_count", 0)
        badge.text = cartCount.toString()
        Log.d("cartCount cartCount", "onCreateView : "+cartCount)
        ...
        // Food RecyclerView (already implemented by you)
        val json = readRawText(this, R.raw.restaurantdata)
        ...
        //1) Build tags (no "All") Collect all tags from all restaurants
        val allTags = restaurantList
            .flatMap { it.food ?: emptyList() }            // food list extract karo
            .flatMap { it?.tags ?: emptyList() }            // tags extract karo
            .distinct()                                    // duplicates remove

        Log.d("UniqueTags", allTags.joinToString())


        // 2) Food adapter
//        val foodAdapter = FoodAdapter(foodList.toMutableList())
        ...
        // 3) Filter helper (no "All" case)
        fun applyTag(tag: String) {
            val filtered = foodList.filter { it.tags?.contains(tag) == true }
            foodAdapter.updateData(filtered)   // or update(filtered) — match your adapter
        }

        // 4) Category adapter
        val categoryAdapter = FoodCategoryAdapter(allTags) { selected ->
            applyTag(selected)                 // update on click
        }
        recyclerCategories.layoutManager =
            LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        recyclerCategories.adapter = categoryAdapter

        // 5) Initial selection (pick your default tag)
        val initialTag = allTags.firstOrNull() ?: return  // or "Recommended" if present
        applyTag(initialTag)
        // (optional) highlight selected chip if your adapter supports it)
        categoryAdapter.setSelectedIndex(0)*/
